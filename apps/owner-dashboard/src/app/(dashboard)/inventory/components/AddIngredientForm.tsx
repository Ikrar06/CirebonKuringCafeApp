...
export default function AddIngredientForm({ onClose, onSuccess }: AddIngredientFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const code = formData.code || `ING-${Date.now().toString().slice(-6)}`
      ...
      setErrors({ submit: error.message || 'Failed to create ingredient. Please try again.' })
    } finally {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            >
              ...
              <span>{isSubmitting ? 'Creating...' : 'Create Ingredient'}</span>
            </button>
          </div>
        </form>
      </div>
